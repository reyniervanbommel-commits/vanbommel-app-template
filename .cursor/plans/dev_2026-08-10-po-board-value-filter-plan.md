...
**User story:**
Als inkoper wil ik bij kolomfilters `equals` en `is one of` een lijst van beschikbare waarden zien
en meerdere waarden als chips kunnen invoeren of plakken, zodat ik snel en foutloos op bekende
waarden kan filteren zonder exact te moeten typen.

**Acceptatiecriteria (globaal — Definition of Done):**
- [ ] `equals`-operator toont een typeahead-dropdown met unieke kolomwaarden; klikken vult het veld.
- [ ] `oneOf`-operator slaat waarden op als array; plakken van meerdere regels voegt meerdere chips toe.
- [ ] Suggesties zijn cascading: ze respecteren actieve filters op andere kolommen (kleurfilters uitgesloten).
- [ ] Een opgeslagen view met legacy komma-string laadt en filtert correct (backward compat).
- [ ] Date-kolommen zijn ongewijzigd; alle bestaande tests blijven groen.
- [ ] Versienummer is verhoogd in `src/config/version.js`.
...
## Prerequisite Task 1: `tableViewFilterUtils.js` splitsen

**Waarom:** Het bestand staat op 254 regels. Tasks 1–3 voegen ~90 regels toe → ~344 totaal (hard
stop per code-kwaliteitsregel ≥ 300). Verplaats de date-specifieke helpers naar een eigen bestand
zodat er na de split voldoende ruimte is voor de filter-uitbreidingen.

**Files:**
- Create: `src/utils/dateFilterUtils.js`
- Modify: `src/utils/tableViewFilterUtils.js` (verwijder date-helpers, voeg import toe)

**Steps:**

- [ ] **Step 1: Verplaats date-helpers**

  Verplaats de volgende functies (~55 regels) naar `src/utils/dateFilterUtils.js` als named
  exports: `parseDateValue`, `startOfToday`, `startOfNextWeek`, `dateMatchesFilter`.

- [ ] **Step 2: Update imports in `tableViewFilterUtils.js`**

  Vervang de lokale implementaties door:
  ```js
  import { dateMatchesFilter } from './dateFilterUtils';
  ```
  De helper-functies `parseDateValue`/`startOfToday`/`startOfNextWeek` zijn alleen intern gebruikt
  door `dateMatchesFilter` — ze hoeven niet apart geïmporteerd te worden.

- [ ] **Step 3: Controleer regelcount**

  `tableViewFilterUtils.js` moet na de verplaatsing < 220 regels zijn (ruimte voor Tasks 1–3).

- [ ] **Step 4: Run tests**

  Run: `npm test -- tableViewFilterUtils`
  Expected: PASS (geen gedragswijziging).

- [ ] **Step 5: Commit**

  ```bash
  git add src/utils/dateFilterUtils.js src/utils/tableViewFilterUtils.js
  git commit -m "refactor(po-filter): extraheer date-filterhulpfuncties naar dateFilterUtils"
  ```
...
export default function PurchaseOrderColumnFilterValuePicker({
  ...
  // Sync inputText wanneer de parent de value extern reset (bijv. bij operator-switch).
  useEffect(() => {
...
## Prerequisite Task 8: `PurchaseOrdersBoardTable.jsx` splitsen

**Waarom:** Het bestand staat op 330 regels (hard stop per code-kwaliteitsregel). Task 8 voegt een
`items`-prop toe aan dit bestand. Een autonome agent stopt bij het stop-signaal — de split moet
eerst plaatsvinden.

**Files:**
- Create: `src/components/supplier/PurchaseOrdersBoardTableHeader.jsx`
- Modify: `src/components/supplier/PurchaseOrdersBoardTable.jsx`

**Steps:**

- [ ] **Step 1: Extraheer de header-logica**

  Verplaats het `<PurchaseOrdersBoardHeaderRow>`-renderdeel (inclusief de directe
  prop-doorgave-logica) naar een nieuw component `PurchaseOrdersBoardTableHeader.jsx`.
  Het nieuwe component accepteert dezelfde props die `PurchaseOrdersBoardTable` nu direct doorgeeft
  aan `PurchaseOrdersBoardHeaderRow`.

- [ ] **Step 2: Controleer regelcount**

  `PurchaseOrdersBoardTable.jsx` moet na de extractie < 280 regels zijn.
...
- [ ] **Step 4: Commit**
  ...
  ```bash
  git add src/components/supplier/PurchaseOrdersBoardTableHeader.jsx src/components/supplier/PurchaseOrdersBoardTable.jsx
  git commit -m "refactor(po-filter): extraheer PurchaseOrdersBoardTableHeader uit BoardTable"
  ```
...
> **Let op (code-kwaliteitsregel):** `PurchaseOrderColumnFilterMenu.jsx` staat op 282 regels.
> Controleer na deze stap de regelcount. Als het bestand ≥ 300 regels bereikt, extraheer dan de
> unieke-waarden-logica naar een custom hook `src/hooks/useUniqueColumnValues.js` die de `useMemo`
> + `EMPTY_UNIQUE_VALUES` bevat, vóór je verdergaat met de rest van Task 8.
> Signatuur: `useUniqueColumnValues(column, items, referenceColumns, allFilters, allDatePeriodDisplayModes, open, isDate)`
...
- [ ] **Step 4b: Versie verhogen**

Verhoog het versienummer in `src/config/version.js` (PATCH-bump).

```bash
git add src/config/version.js
git commit -m "chore(version): bump versienummer na po-board value-filter feature"
```
